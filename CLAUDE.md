...
**Auth** (`src/lib/auth.ts`) is **Better Auth** with the Prisma adapter. Session/User/Account/Verification models live in `prisma/schema.prisma` alongside the domain models. There is **no transactional email provider wired up** — email verification and password reset are intentionally disabled. Signup completes immediately and lands the user on `/dashboard`. If you re-introduce email, also re-enable `requireEmailVerification` and the `sendResetPassword` / `sendVerificationEmail` callbacks. Google + GitHub social providers are conditionally enabled based on `*_CLIENT_ID` / `*_CLIENT_SECRET` env vars; they silently disappear when unset. `baseURL` resolves to `BETTER_AUTH_URL` if set, otherwise to `https://${VERCEL_PROJECT_PRODUCTION_URL}` on production deploys, `https://${VERCEL_URL}` on previews, and `http://localhost:3000` locally. `TRUSTED_ORIGINS` is a comma-separated env var merged into the trusted-origins list.
